...
void collision(int key)
{
	...
	for (int i = 0; i < 15; i++)
	{
		...
		for (auto j : Room[clients[key].avatar.playroom].blueminion)
		{
			if (j.is_alive == false) continue;
			if (clients[key].avatar.bullet[i].position.x > j.object.GetPosition().x - 10
				&& clients[key].avatar.bullet[i].position.x < j.object.GetPosition().x + 10
				&& clients[key].avatar.bullet[i].position.z > j.object.GetPosition().z - 10
				&& clients[key].avatar.bullet[i].position.z < j.object.GetPosition().z + 10
				//&& clients[key].avatar.bullet[i].position.y > j.object.GetPosition().y - 100
				//&& clients[key].avatar.bullet[i].position.y < j.object.GetPosition().y + 100)
				)
			{
				cout << "충돌했습니다" << endl;
...
void roomrate(int key)
{
	...
	if (msec.count() > 5000 && Room[key].minionindex < 40) // 미니언 6마리 스폰  코드
	{
		cout << endl;
		...
		for (int i = 0; i < 2; i++)
		{
			...
			if (!Room[key].redminion[Room[key].minionindex].is_alive)
			{
				...
				cout << "[" << key << "]" << Room[key].minionindex++ << endl;
	...
	for (int i = 0; i < 100; ++i)
	{
		if (Room[key].redminion[i].is_alive)
		{
			...
			if (0 == Room[key].redminion[i].line)
			{
				...
				{
					cout << "레드 [" << i << "]" << " 도착" << endl;
					Room[key].redminion[i].is_alive = false;
				}
			...
			astar_lock.lock();
			Room[key].redminion[i].route = pathFind(xA, yA, xB, yB);
			astar_lock.unlock();
			if (Room[key].redminion[i].route.length() > 0)
			{
				int j; char c;
				c = Room[key].redminion[i].route.at(0);
				j = atoi(&c);
	...
	for (int i = 0; i < 100; ++i)
	{
		if (Room[key].blueminion[i].is_alive)
		{
			...
			if (0 == Room[key].blueminion[i].line)
			{
				...
				{
					cout << "블루 [" << i << "]" << " 도착" << endl;
					Room[key].blueminion[i].is_alive = false;
				}
			...
			for (auto j : Room[key].clients_list)
			{
				...
				{
					if (abs(Room[key].blueminion[i].object.GetPosition().x - clients[j].avatar.object.GetPosition().x) < 200)
					{
						if (abs(Room[key].blueminion[i].object.GetPosition().z - clients[j].avatar.object.GetPosition().z) < 200)
						{
							xB = clients[j].avatar.object.GetPosition().x;
							yB = clients[j].avatar.object.GetPosition().z;
							indextonode(xB, yB);
						...
						{
							PlayerNum = j;
							Room[key].blueminion[i].inplayerarea = 1;
						}
						else
						{
							PlayerNum = -1;
							Room[key].blueminion[i].inplayerarea = 0;
						}
					}
			...
			astar_lock.lock();
			Room[key].blueminion[i].route = pathFind(xA, yA, xB, yB);
			astar_lock.unlock();
			if (Room[key].blueminion[i].route.length() > 0)

			{
				int j; char c;
				c = Room[key].blueminion[i].route.at(0);
				j = atoi(&c);
			...
			else if (Room[key].blueminion[i].inplayerarea == 1)
			{
				if (Room[key].blueminion[i].object.GetPosition().x > clients[PlayerNum].avatar.object.GetPosition().x)
					Room[key].blueminion[i].object.m_d3dxmtxLocal._41 -= 1;
				if (Room[key].blueminion[i].object.GetPosition().x < clients[PlayerNum].avatar.object.GetPosition().x)
					Room[key].blueminion[i].object.m_d3dxmtxLocal._41 += 1;
			}
			sc_packet_minionpos send_packet;
