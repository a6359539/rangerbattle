...
void CGameFramework::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		...
		case VK_F6:
			...
			waveform.nSamplesPerSec =20000;
			waveform.nAvgBytesPerSec =20000;
...
LRESULT CALLBACK CGameFramework::OnProcessingWindowMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case MM_WIM_CLOSE:
	{
		...
		pcodebuffer = reinterpret_cast <char*> (malloc(50000));
		for (int i = 0; i < dwDataLength; i += 160)
		{

			for (int j = 0; j < 160; j++)
				input[j] = pSaveBuffer[j + sumByte];
			speex_bits_reset(&bits);

			speex_encode(enc_state, input, &bits);
			nbBytes = speex_bits_write(&bits, cbits, 200);
			//printf("%d \n", nbBytes);
			//pcodebuffer = pSaveBuffer;
			for (int j = 0; j < nbBytes; j++)
				pcodebuffer[j + sumsumsum] = cbits[j];
			sumsumsum += nbBytes;
			sumByte += 160;
		}
		...
		sumByte = 0;
		go = reinterpret_cast <PBYTE> (malloc(50000));
		int nb = 0;

		speex_bits_init(&decbits);
		for (int i = 0; i < sumsumsum; i += nbBytes) {
			for (int j = 0; j < nbBytes; j++)
			{
				cbits[j] = pcodebuffer[j + i];
			}
			speex_bits_read_from(&decbits, cbits, nbBytes);
			speex_decode_int(dec_state, &decbits, out);
			for (int j = 0; j < 160; j++) {
	...
	case MM_WOM_OPEN:
		printf("이거실행해쓰요!");
		pWaveHdr1->lpData = reinterpret_cast <CHAR*>(go);
