...
//test
struct VS_SRV_BUFFER_ANIMATION
{
	D3DXMATRIX m_d3dxmtxAnimation[MAX_BONE_COUNT];
};
struct VS_CB_ANIMATION_MATRIX
{
	D3DXMATRIX m_d3dmtxAnimation;
};
...
public:
	...
	void CreateAnimationBuffer(ID3D11Device* pd3dDevice);
	virtual void CreateShader(ID3D11Device *pd3dDevice);
	virtual void BuildObjects(ID3D11Device *pd3dDevice, CMarineMeshDiffused *pMesh);

	virtual void UpdateAnimationBuffer(ID3D11DeviceContext *pd3dDeviceContext, D3DXMATRIX *pd3dxmtxAnimation, D3DXMATRIX *pd3dxmtxLocal, int nBone);
	virtual void AnimateObjects(float fTimeElapsed);
	virtual void Render(ID3D11DeviceContext *pd3dDeviceContext, D3DXMATRIX* playermtx);


public:
	CAnimationObject	**m_ppObjects{ nullptr };
	int					m_nObjects{ 0 };

	ID3D11Buffer*		m_pd3dcbAnimation{ nullptr };
...
private:
	ID3D11SamplerState			*m_pd3dModelSamplerState{ nullptr };
	ID3D11ShaderResourceView	*m_pd3dModelTexture{ nullptr };
